...
import 'package:khedma/widgets/custom_loading.dart';
  ...
  Future handleGoogleSignIn(
    ...
    try {
      ...
      if (auth.accessToken != null) {
        ...
        logError("accessToken: ${auth.accessToken}");
        logError("fcsToken: $fcsToken");
  ...
  Future<LoginStates> handleNormalSignIn({
    ...
    try {
      ...
      if (res.data["access_token"] != null) {
        String token = res.data["access_token"];
        String type = res.data["role_type"];
        bool data = res.data["complite_data"];
        _globalController.dataUser = data;
        _globalController.typeUser = type;
        await Utils.saveToken(token: token);
        await Utils.saveRemmemberMe(rem: saveToken ? "yes" : "no");
